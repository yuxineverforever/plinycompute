...
int main(int argc, char* argv[]) {
    ...
    if (whetherToAddData) {
        ...
        pdbClient.createDatabase("LA08_db");

        // now, create a new set in that database
        pdbClient.createSet<MatrixBlock>("LA08_db", "LA_input_set");
        ...
        pdbClient.sendData<MatrixBlock>("LA08_db", "LA_input_set", storeMe);
        PDB_COUT << blockSize << "MB data sent to dispatcher server~~" << std::endl;
    }
    // now, create a new set in that database to store output data

    PDB_COUT << "to create a new set for storing output data" << std::endl;
    pdbClient.createSet<MatrixBlock>("LA08_db", "LA_rowMax_set");
    ...
    pdbClient.registerType("libraries/libLARowMaxAggregate.so");
    ...
    Handle<Computation> myQuery = makeObject<LARowMaxAggregate>();
    ...
    if (printResult) {
        ...
        auto input_iter = pdbClient.getSetIterator<MatrixBlock>("LA08_db", "LA_input_set");
        ...
        auto output_iter = pdbClient.getSetIterator<MatrixBlock>("LA08_db", "LA_rowMax_set");
        std::cout << "Row max query results: " << std::endl;
        int countOut = 0;
        ...
        std::cout << "Row max output count:" << countOut << "\n";
    }
    pdbClient.removeSet("LA08_db", "LA_rowMax_set");
