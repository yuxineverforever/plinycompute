...
// by Binhang, May 2017
// to test matrix subtract implemented by join;
#include <ctime>
#include <chrono>
...
int main(int argc, char* argv[]) {
    ...
    pdbClient.registerType("libraries/libLASubtractJoin.so");
    ...
    Handle<Computation> mySubtractJoin = makeObject<LASubtractJoin>();
    mySubtractJoin->setInput(0, myMatrixSet1);
    mySubtractJoin->setInput(1, myMatrixSet2);

    Handle<Computation> mySumWriteSet = makeObject<LAWriteMatrixBlockSet>("LA03_db", "LA_diff_set");
    mySumWriteSet->setInput(mySubtractJoin);
    ...
    if (printResult) {
        ...
        std::cout << "Subtract query results: " << std::endl;
        int countOut = 0;
