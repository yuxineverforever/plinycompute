 ...
 *                                                                           *
 *****************************************************************************/
#ifndef TEST_LA_10_CC
#define TEST_LA_10_CC


// by Binhang, May 2017
// to test matrix colMax implemented by aggregation;
#include <ctime>
#include <chrono>
...
int main(int argc, char* argv[]) {
    ...
    if (whetherToAddData) {
        ...
        pdbClient.createDatabase("LA10_db");

        // now, create a new set in that database
        pdbClient.createSet<MatrixBlock>("LA10_db", "LA_input_set");
        ...
        pdbClient.sendData<MatrixBlock>("LA10_db", "LA_input_set", storeMe);
        PDB_COUT << blockSize << "MB data sent to dispatcher server~~" << std::endl;
    }
    // now, create a new set in that database to store output data

    PDB_COUT << "to create a new set for storing output data" << std::endl;
    pdbClient.createSet<MatrixBlock>("LA10_db", "LA_colMax_set");
    ...
    pdbClient.registerType("libraries/libLAColMaxAggregate.so");
    ...
    Handle<Computation> myWriteSet = makeObject<LAWriteMatrixBlockSet>("LA10_db", "LA_colMax_set");
    ...
    if (printResult) {
        ...
        auto input_iter = pdbClient.getSetIterator<MatrixBlock>("LA10_db", "LA_input_set");
        ...
        auto output_iter = pdbClient.getSetIterator<MatrixBlock>("LA10_db", "LA_colMax_set");
        std::cout << "Col max query results: " << std::endl;
        int countOut = 0;
        ...
        std::cout << "Col max output count:" << countOut << "\n";
    }
    pdbClient.removeSet("LA10_db", "LA_colMax_set");
