...
int main(int argc, char* argv[]) {
    ...
    if (whetherToAddData) {
        ...
        pdbClient.createDatabase("LA15_db");

        // now, create a new set in that database
        pdbClient.createSet<MatrixBlock>("LA15_db", "LA_input_set");
        ...
        // Write 100 Matrix of size 40 * 20
        int matrixRowNums = 4;
        int matrixColNums = 4;
        int blockRowNums = 10;
        int blockColNums = 5;
        ...
        pdbClient.sendData<MatrixBlock>("LA15_db", "LA_input_set", storeMe);
        PDB_COUT << blockSize << "MB data sent to dispatcher server~~" << std::endl;
    }
    // now, create a new set in that database to store output data

    PDB_COUT << "to create a new set for storing output data" << std::endl;
    pdbClient.createSet<MatrixBlock>("LA15_db", "LA_rowSum_set");
    ...
    pdbClient.registerType("libraries/libLARowSumAggregate.so");
    pdbClient.registerType("libraries/libLAScanMatrixBlockSet.so");
    pdbClient.registerType("libraries/libLAWriteMatrixBlockSet.so");



    Handle<Computation> myScanSet = makeObject<LAScanMatrixBlockSet>("LA15_db", "LA_input_set");
    Handle<Computation> myQuery = makeObject<LARowSumAggregate>();
    myQuery->setInput(myScanSet);

    Handle<Computation> myWriteSet = makeObject<LAWriteMatrixBlockSet>("LA15_db", "LA_rowSum_set");
    ...
    if (printResult) {
        ...
        auto input_iter = pdbClient.getSetIterator<MatrixBlock>("LA15_db", "LA_input_set");
        ...
        auto output_iter = pdbClient.getSetIterator<MatrixBlock>("LA15_db", "LA_rowSum_set");
        std::cout << "Row sum query results: " << std::endl;
        int countOut = 0;
        ...
        std::cout << "Row sum output count:" << countOut << "\n";
    }
    pdbClient.removeSet("LA15_db", "LA_rowSum_set");
