...
int main(int argc, char* argv[]) {
    ...
    if (whetherToAddData) {
        ...
        pdbClient.createDatabase("LA16_db");

        // now, create a new set in that database
        pdbClient.createSet<MatrixBlock>("LA16_db", "LA_input_set");
        ...
        pdbClient.sendData<MatrixBlock>("LA16_db", "LA_input_set", storeMe);
        PDB_COUT << blockSize << "MB data sent to dispatcher server~~" << std::endl;
    }
    // now, create a new set in that database to store output data

    PDB_COUT << "to create a new set for storing output data" << std::endl;
    pdbClient.createSet<MatrixBlock>("LA16_db", "LA_colSum_set");
    ...
    Handle<Computation> myWriteSet = makeObject<LAWriteMatrixBlockSet>("LA16_db", "LA_colSum_set");
    ...
    if (printResult) {
        ...
        auto input_iter = pdbClient.getSetIterator<MatrixBlock>("LA16_db", "LA_input_set");
        ...
        auto output_iter = pdbClient.getSetIterator<MatrixBlock>("LA16_db", "LA_colSum_set");
        std::cout << "Col sum query results: " << std::endl;
        int countOut = 0;
        ...
        std::cout << "Col sum output count:" << countOut << "\n";
    }
    pdbClient.removeSet("LA16_db", "LA_colSum_set");
