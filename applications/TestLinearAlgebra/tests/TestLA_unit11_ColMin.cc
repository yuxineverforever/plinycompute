 ...
 *                                                                           *
 *****************************************************************************/
#ifndef TEST_LA_11_CC
#define TEST_LA_11_CC
...
int main(int argc, char* argv[]) {
    ...
    if (whetherToAddData) {
        ...
        pdbClient.createDatabase("LA11_db");

        // now, create a new set in that database
        pdbClient.createSet<MatrixBlock>("LA11_db", "LA_input_set");
        ...
        pdbClient.sendData<MatrixBlock>("LA11_db", "LA_input_set", storeMe);
        PDB_COUT << blockSize << "MB data sent to dispatcher server~~" << std::endl;
    }
    // now, create a new set in that database to store output data

    PDB_COUT << "to create a new set for storing output data" << std::endl;
    pdbClient.createSet<MatrixBlock>("LA11_db", "LA_colMin_set");
    ...
    pdbClient.registerType("libraries/libLAColMinAggregate.so");
    pdbClient.registerType("libraries/libLAScanMatrixBlockSet.so");
    pdbClient.registerType("libraries/libLAWriteMatrixBlockSet.so");

    Handle<Computation> myScanSet = makeObject<LAScanMatrixBlockSet>("LA11_db", "LA_input_set");
    Handle<Computation> myQuery = makeObject<LAColMinAggregate>();
    myQuery->setInput(myScanSet);
    // myQuery->setOutput("LA08_db", "LA_rowMax_set");

    Handle<Computation> myWriteSet = makeObject<LAWriteMatrixBlockSet>("LA11_db", "LA_colMin_set");
    ...
    if (printResult) {
        ...
        auto input_iter = pdbClient.getSetIterator<MatrixBlock>("LA11_db", "LA_input_set");
        ...
        auto output_iter = pdbClient.getSetIterator<MatrixBlock>("LA11_db", "LA_colMin_set");
        std::cout << "Col min query results: " << std::endl;
        int countOut = 0;
        ...
        std::cout << "Col min output count:" << countOut << "\n";
    }
    pdbClient.removeSet("LA11_db", "LA_colMin_set");
