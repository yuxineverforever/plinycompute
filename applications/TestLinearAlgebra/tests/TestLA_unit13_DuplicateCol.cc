 ...
 *                                                                           *
 *****************************************************************************/
#ifndef TEST_LA_13_CC
#define TEST_LA_13_CC
...
int main(int argc, char* argv[]) {
    ...
    if (whetherToAddData) {
        ...
        pdbClient.createDatabase("LA13_db");

        // now, create a new set in that database
        pdbClient.createSet<MatrixBlock>("LA13_db", "LA_input_set");
        ...
        // Write 100 Matrix of size 50 * 1
        int matrixRowNums = 10;
        int matrixColNums = 1;
        int blockRowNums = 5;
        int blockColNums = 1;
        ...
        pdbClient.sendData<MatrixBlock>("LA13_db", "LA_input_set", storeMe);
        PDB_COUT << blockSize << "MB data sent to dispatcher server~~" << std::endl;
    }
    // now, create a new set in that database to store output data

    PDB_COUT << "to create a new set for storing output data" << std::endl;
    pdbClient.createSet<MatrixBlock>("LA13_db", "LA_duplicateCol_set");
    ...
    pdbClient.registerType("libraries/libLADuplicateColMultiSelection.so");
    pdbClient.registerType("libraries/libLAScanMatrixBlockSet.so");
    pdbClient.registerType("libraries/libLAWriteMatrixBlockSet.so");

    Handle<Computation> myScanSet = makeObject<LAScanMatrixBlockSet>("LA13_db", "LA_input_set");
    LADimension targetDim(5, 5, 10, 10);
    Handle<Computation> myQuery = makeObject<LADuplicateColMultiSelection>(targetDim);
    myQuery->setInput(myScanSet);

    Handle<Computation> myWriteSet =
            makeObject<LAWriteMatrixBlockSet>("LA13_db", "LA_duplicateCol_set");
    ...
    if (printResult) {
        ...
        auto input_iter = pdbClient.getSetIterator<MatrixBlock>("LA13_db", "LA_input_set");
        ...
        auto output_iter = pdbClient.getSetIterator<MatrixBlock>("LA13_db", "LA_duplicateCol_set");
        std::cout << "Duplicate col query results: " << std::endl;
        int countOut = 0;
        ...
        std::cout << "Duplicate col output count:" << countOut << "\n";
    }
    pdbClient.removeSet("LA13_db", "LA_duplicateCol_set");
